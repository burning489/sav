...
def r_init(phi, **kwargs):
    ...
    Nx, Ny, Lx, Ly, C = kwargs["Nx"], kwargs["Ny"], kwargs["Lx"], kwargs[
        "Ly"], kwargs["C"]
    hx, hy = Lx / Nx, Ly / Ny
...
def compute_b(phi, **kwargs):
    ...
    return F_der(
        phi, **
        kwargs) / np.sqrt(np.fft.fft2(F(phi, **kwargs))[0, 0] * hx * hy + C)


def compute_g(phi0, r0, b, **kwargs):
    """Compute righthand side g"""
    Nx, Ny, Lx, Ly, dt = kwargs["Nx"], kwargs["Ny"], kwargs["Lx"], kwargs[
        "Ly"], kwargs["dt"]
    hx, hy = Lx / Nx, Ly / Ny
    bphi0 = np.fft.fft2(b * phi0)[0, 0] * hx * hy
